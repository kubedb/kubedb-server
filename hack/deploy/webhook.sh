...
curl -fsSL https://raw.githubusercontent.com/kubedb/admission-webhook/master/hack/deploy/operator.yaml | envsubst | kubectl apply -f -
...
if [ "$KUBEDB_ENABLE_RBAC" = true ]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/kubedb/admission-webhook/master/hack/deploy/rbac-list.yaml | envsubst | kubectl auth reconcile -f -
fi
